              ...
              <tbody>
                {employees?.map(emp => (
                  <tr key={emp.id} className="hover:bg-white/2 transition-colors group">
                    ...
                    <Td className="font-medium text-white font-mono text-xs">{formatCurrency(emp.salary)}</Td>
                    <Td>
...
type EmployeeForm = {
  ...
  status: EmployeeStatus; bio: string
}

function emptyForm(): EmployeeForm {
  return {
    first_name: '', last_name: '', email: '', phone: '',
    department: 'Engineering', job_title: '', location: '',
    start_date: '', salary: '', employment_type: 'full_time', status: 'active', bio: '',
  }
}
    ...
    <Modal open={true} onClose={onClose} title="Employee Details" size="lg">
      <div className="space-y-5">
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-4">
          ...
          <DetailRow label="Annual Salary" value={formatCurrency(employee.salary)} highlight />
        </div>
