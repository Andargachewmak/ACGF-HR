...
import type { Department, EmploymentType, CandidateStage } from '@/types'

const DEPARTMENTS: Department[] = ['Engineering', 'Sales', 'Marketing', 'Design', 'Product', 'Operations', 'HR', 'Finance', 'Legal']
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <StatCard label="Active Applicants" value={totalApplicants} change="+38 this week" changeType="up" icon="👤" accent="bg-teal-500" />
        <StatCard label="Avg. Time to Hire" value="24d" change="Down from 31 days" changeType="up" icon="⏱" accent="bg-amber-500" />
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <div className="flex items-center gap-2">
            <Badge status="active" className="bg-brand-500/15 text-brand-400 border-brand-500/20">{totalApplicants} candidates</Badge>
        ...
        <CardBody>
          <div className="grid grid-cols-5 gap-3">
            {PIPELINE_STAGES.map(stage => (
              <div key={stage.name} className="bg-surface-2 rounded-xl p-4">
                ...
                <div className="mt-2 h-1 rounded-full bg-white/5">
                  <div className="h-full rounded-full" style={{ width: `${(stage.count / PIPELINE_STAGES[0].count) * 100}%`, background: stage.color }} />
                </div>
              </div>
            ))}
          </div>
...
function PostJobModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [form, setForm] = useState({
    title: '', department: 'Engineering' as Department, location: 'Remote',
  ...
  async function submit() {
    ...
    try {
      ...
      onClose()
      setForm({ title: '', department: 'Engineering', location: 'Remote', employment_type: 'full_time', salary_min: '', salary_max: '', description: '', requirements: '' })
    } catch (e) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Post a job" size="lg">
      <div className="grid grid-cols-2 gap-4">
        ...
        <Select label="Department" value={form.department} onChange={v => update('department', v as Department)} options={DEPARTMENTS.map(d => ({ value: d, label: d }))} />
